...
class SiteTableViewController: UITableViewController {
  ...
  override func tableView(_ tableView: UITableView, moveRowAt fromIndexPath: IndexPath, to toIndexPath: IndexPath) {
    ...
    let objectToMove = fetchedArticlesController.object(at: ffi) as Article
    let objectToDisplace = fetchedArticlesController.object(at: fti) as Article
    ...
    if toIndexPath.row == 0 {
      ...
      let objectBeforeDest = fetchedArticlesController.object(at: fi) as Article
  ...
  override func decodeRestorableState(with coder: NSCoder) {
    ...
    do {
      annotation = try sharedContext.fetch(request).first as? VTAnnotation
    } catch {
      print("could not fetch VTAnnotation")
    }
